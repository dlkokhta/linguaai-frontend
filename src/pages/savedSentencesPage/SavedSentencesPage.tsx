...
export const SavedSentencesPage = () => {
  ...
  const { data: sentences = [], isLoading } = useQuery({
    queryKey: ["saved-sentences"],
    queryFn: () => axiosInstance.get<SavedSentence[]>("/saved-sentences").then((r) => r.data),
  });

  const deleteMutation = useMutation({
    mutationFn: (id: string) => axiosInstance.delete(`/saved-sentences/${id}`),
    onSuccess: (_, id) => {
      queryClient.setQueryData<SavedSentence[]>(["saved-sentences"], (prev) =>
